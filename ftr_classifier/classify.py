...
def _append_spacy_docs(df, lang_col, text_col):
    ...
    if lang_col == "english_all":
        ...
        df['spacy_doc'] = df[text_col].apply(lambda x: nlp(x) if isinstance(x, str) else nlp(""))
        # append together
        return df
        # dx = pd.concat([dx, dy])
    else:
...
def _clean_non_applicable(df, lang_col):
    ...
    if lang_col == "english_all":
        ...
        cols = [x for x in df.columns if 'particle' in x]
        df[cols] = _MISSING
        return df
    else:
        df_group = df.groupby(lang_col)
...
def _aspect_dom(df, lang_col='language'):
    store = pd.DataFrame()
    if lang_col == "english_all":
        df['present_perfect'] = [1 if df.present_perfect[x] == 1 and df.past[x] == 1 and df.future[x] == 0 else 0
                                 for x in df.index]
        df['past_perfect'] = [1 if df.past_perfect[x] == 1 and df.past[x] == 1 and df.future[x] == 0 else 0 for x in
                              df.index]
        return df
        ...
        for lang, dx in dfg:
            if lang == 'english':
                dx['present_perfect'] = [
                    1 if dx.present_perfect[x] == 1 and dx.past[x] == 1 and dx.future[x] == 0 else 0 for x in dx.index]
                dx['past_perfect'] = [1 if dx.past_perfect[x] == 1 and dx.past[x] == 1 and dx.future[x] == 0 else 0 for
                                      x in dx.index]
                store = pd.concat([store, dx])
            elif lang == 'dutch':
                dx['present_perfect'] = [
                    1 if dx.present_perfect[x] == 1 and dx.past_participle[x] == 1 and dx.future[x] == 0 else 0 for x in
                    dx.index]
                dx['past_perfect'] = [
                    1 if dx.past_perfect[x] == 1 and dx.past_participle[x] == 1 and dx.future[x] == 0 else 0 for x in
                    dx.index]
                store = pd.concat([store, dx])
            else:
                store = pd.concat([store, dx])
...
def _modal_exclude(df):
    ...
    df[new_val] = [new_val if any(feature == 1 for feature in df.loc[x, certs]) and \
                              any(feature == 1 for feature in df.loc[x, posses]) else "unmixed_modality" for x in
                   df.index]
    return df
...
def _make_lexi_vars(df,experimental_data):
    ...
    df['uncertain'] = [1 if any(feature == 1 for feature in df.loc[x, poss_features]) \
                           else "mixed_modality" if any(
        feature == "mixed_modality" for feature in df.loc[x, poss_features])
    else 0 for x in df.index]
    df['certain'] = [1 if any(feature == 1 for feature in df.loc[x, cert_features]) \
                         else "mixed_modality" if any(
        feature == "mixed_modality" for feature in df.loc[x, cert_features])
    else 0 for x in df.index]
